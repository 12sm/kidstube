...
1. On the Roku remote from the home screen: **Home×3, Up×2, Right, Left, Right, Left, Right** — this opens developer mode. Enable it, set password `(device password)`. Note the device IP shown on screen.
...
<component name="ProfileSelect" extends="Scene">
    ...
    <children>
        ...
        <Rectangle id="child1Card" width="220" height="260" color="0x2D2D5EFF" translation="[290, 260]" focusable="true" />
        <Label id="child1Label" text="Child1" width="220" horizAlign="center"
               translation="[290, 530]" color="0xFFFFFFFF" font="font:MediumBoldSystemFont" />
        <Rectangle id="child2Card" width="220" height="260" color="0x2D2D5EFF" translation="[770, 260]" focusable="true" />
        <Label id="child2Label" text="Child2" width="220" horizAlign="center"
...
```brightscript
sub init()
    m.child1Card = m.top.findNode("child1Card")
    m.child2Card = m.top.findNode("child2Card")
    m.top.setFocus(true)
    m.child1Card.setFocus(true)
end sub
function onKeyEvent(key as String, press as Boolean) as Boolean
    if press
        if key = "OK"
            if m.child1Card.hasFocus() then selectProfile(5, "Child1")
            if m.child2Card.hasFocus() then selectProfile(6, "Child2")
            return true
        else if key = "right" and m.child1Card.hasFocus()
            m.child2Card.setFocus(true)
            m.child2Card.color = "0x4A4A8EFF"
            m.child1Card.color = "0x2D2D5EFF"
            return true
        else if key = "left" and m.child2Card.hasFocus()
            m.child1Card.setFocus(true)
            m.child1Card.color = "0x4A4A8EFF"
            m.child2Card.color = "0x2D2D5EFF"
            return true
        end if
    end if
...
App Start → ProfileSelect (scene root)
  D-pad left/right to toggle Child1 / Child2, OK to select
