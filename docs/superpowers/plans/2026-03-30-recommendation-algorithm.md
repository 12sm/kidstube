...
beforeAll(() => {
  ...
  seedProfile(db, { id: 1, name: 'Weston' });
...
test('falls back to recency when profile has no interests', () => {
  seedProfile(db, { id: 2, name: 'Emery' });
 ...
 *
 * @param {string} profileName - e.g. "Weston"
...
Expected: `{"ok":true,"markdown":"Weston is 8 years old..."}` and parent interests in profile_interests table.
