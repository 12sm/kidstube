...
zip -r ../roku-app.zip .
curl -s -S --user rokudev:kidstube123 --anyauth \
  -F "mysubmit=Install" \
...
- `"network error"`: the Roku can't reach the backend or YouTube CDN. Verify `m.global.backendUrl` in `main.brs` is still `http://192.168.1.221:3001`.
